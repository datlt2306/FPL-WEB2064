...
const checkAuth = () => {
    if (!isAuthenticated() && !window.location.pathname.includes('signin') && !window.location.pathname.includes('signup')) {
    ...
    try {
        ...
        productList.innerHTML = products.map((product, index) => `
            <tr>
                ...
                <td>
                    <a href="./edit.html?id=${product.id}" class="btn btn-sm btn-warning me-2">Sửa</a>
                    <button class="btn btn-sm btn-danger" onclick="handleDeleteProduct('${product.id}')">Xóa</button>
                </td>
            </tr>
        `).join('');
    } catch (error) {
